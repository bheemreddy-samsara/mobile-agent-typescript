...
import { type UIState, VisionMethod } from "../src/types";
...
class MockLLMProvider extends BaseLLMProvider {
  ...
  async query(_prompt: string, _systemPrompt?: string): Promise<string> {
...
describe("LLMProvider Confidence Propagation", () => {
  ...
  describe("Confidence-based fallback decision making", () => {
    ...
    it("should trigger fallback when confidence is exactly 0", () => {
      ...
      const threshold = 0.7;
      const shouldFallback =
        errorResponse.confidence !== undefined && errorResponse.confidence < threshold;
