...
async function run() {
  ...
  if (!appPath && !bundleId) {
    throw new Error(
      "Set MOBILE_APP_PATH (.app) or MOBILE_BUNDLE_ID to run this iOS connectivity test",
    );
  }
  ...
  else capabilities["appium:bundleId"] = bundleId;
  ...
  try {
    ...
    if (!appPath && bundleId) {
      try {
        await (driver as any).activateApp(bundleId);
      } catch {
