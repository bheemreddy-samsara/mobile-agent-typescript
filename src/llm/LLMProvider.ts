...
export interface LLMProvider {
   ...
   */
  generateActionWithVisionTagging(uiState: UIState, instruction: string, history: string[]): Promise<LLMActionResponse>;

  /**
   * Generate an action using vision with grid overlay
   */
  generateActionWithGridOverlay(uiState: UIState, instruction: string, history: string[]): Promise<LLMActionResponse>;
...
export abstract class BaseLLMProvider implements LLMProvider {
  ...
  abstract queryWithVision(prompt: string, imageBase64: string, systemPrompt?: string): Promise<string>;
