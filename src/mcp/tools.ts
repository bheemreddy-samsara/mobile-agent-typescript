    ...
    inputSchema: {
      ...
      properties: {
        instruction: {
          type: "string",
          description:
            'Natural language instruction (e.g., "Click the login button", "Type email@example.com in the email field")',
        },
        visionMode: {
          type: "string",
          enum: ["auto", "pure-vision-only"],
          description:
            'Vision mode: "auto" uses four-tier fallback (recommended), "pure-vision-only" skips to pure vision',
          default: "auto",
        },
      },
    ...
    inputSchema: {
      ...
      properties: {
        condition: {
          type: "string",
          description:
            'Condition to verify (e.g., "Login successful", "User is logged in", "Settings page is displayed")',
        },
      },
      required: ["condition"],
    },
